...
/* Thread d'envois des mix */
 void alarm_handler(int signum){
 	...
 	if(serveur.playing==1){
 		for(i = 0; i < serveur.max_user; ++i){
 			if(serveur.clients[i] != NULL){
 				mix = getMix(serveur.clients[i]);
 				char* mix_string=malloc(sizeof(char)*AUDIO_BUFFER_MAX_SIZE);
 				convertAudioToString(mix, &mix_string);
 				handler_AUDIO_MIX(mix_string, serveur.clients[i]->socket_audio);
 				free(mix_string);
 ...
 void* envoi_mixs(void * args){
	// On boucle, et on attend à chaque tour un SIGALRM pour passer à l'itération
	// d'après.
 	...
 	timer.it_value.tv_usec = 150 * 1000;
 	timer.it_interval.tv_sec = 0;
 	timer.it_interval.tv_usec = 150 * 1000;
 ...
 * Envoi de AUDIO_OK
 * Args :  NULL OU user
 * socket : Socket audio du client OU audio du serveur
 */
 void handler_AUDIO_OK(char * args,int socket){
	...
	// Si on a un argument, c'est la confirmation du port
 	else {
		...
		// dans le handler AUDIO_PORT
 		...
 		int* socket_accept=malloc(sizeof(int));
 		*socket_accept = accept(socket, (struct sockaddr *) &addr_user, &size_usr);
 		if(*socket_accept == -1){
		...
		// On peut lancer un thread d'ecoute
 ...
 */
 void handler_EXITED(char * args,int socket){
	...
	// apres l'admin)
 	if(admin_deco && serveur.nb_user > 0){
 ...
 void handler_AUDIO_CHUNK(char * args,int socket){
	...
	*/
	void handler_AUDIO_KO(char * args,int socket){
		t_client* c=serveur.clients[get_indice_from_socket(socket)];
		c->faillure_audio++;

		if(c->faillure_audio>=8){
			handler_ACCESS_DENIED("connexion trop lente pour l'audio", socket);
			//supprimer_client(c->name);

		}else{
			char audio_ko_cmd[COMMAND_MAX_SIZE];
			sprintf(audio_ko_cmd,"AUDIO_KO/");
			if(send(socket,audio_ko_cmd,strlen(audio_ko_cmd)+1,0)==-1){
				perror("Send audio ko");
			}
			logf("<- %s\n",audio_ko_cmd);
		}	
...
t_audio_buffer* getMix(t_client * client){
 	...
 	client->next_tick_to_send=(client->next_tick_to_send+1)%4;
 	...
 	for(i = 0; i < serveur.max_user; ++i){
 		...
 		if(serveur.clients[i] != client){
 			courrant=pop(&(serveur.clients[i]->queue));

 			if(courrant!=NULL){
 				if(mix->size<courrant->size)
 					mix->size=courrant->size;
 				int j;
 				for(j = 0; j < mix->size; ++j){
 					signal_A = mix->buffer[j];
 					signal_B = courrant->buffer[j];
